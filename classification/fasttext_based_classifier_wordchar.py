...
class FastTextBasedClassifier:
    ...
    def evaluate(self, data, session, eval_nodes):
        ...
        total_pred = []
        n_batch = 0
        for X_batch_charidx, X_batch_mask, X_batch_wordembed, domain_actual_lens, X_batch_suf, sample_weights, y_batch in self.next_batch(data):
            batch_correct, batch_loss, batch_bool, batch_pred = session.run(eval_nodes,
                                                         feed_dict={
                                                                    'bool_train:0': False,
                                                                    'charidx:0': X_batch_charidx,
                                                                    'charidx_mask:0': X_batch_mask,
                                                                    'wordembed:0': X_batch_wordembed,
    ...
    def run_graph(self):
        ...
        x_embed_word = tf.placeholder(tf.float32,
        ...
        embeddings_char = tf.Variable(self.inital_ngram_embed, trainable=not FROZEN)
        embed_char = tf.nn.embedding_lookup(embeddings_char, x_char_ngram_indices)
        mask = tf.expand_dims(mask, axis=-1)

        mask = tf.tile(mask, [1, 1, embed_dimen])
        x_embed_char = tf.multiply(embed_char, mask)



        domain_vectors = []

        ''' apply CNN on char-ngram '''
        if 'CNN' in type:
            pooled_outputs = []
            for filter_size in filter_sizes_char:
                ...
                x_embed_expanded_char = tf.expand_dims(x_embed_char, -1)
                conv = tf.nn.conv2d(x_embed_expanded_char, W_filter, strides=[1, 1, 1, 1], padding="VALID")
                # Apply nonlinearity
        ...
        ''' apply CNN on word embeddings '''
        if 'CNN' in type:
            pooled_outputs = []
            for filter_size in filter_sizes_word:
                # Define and initialize filters
                filter_shape = [filter_size, embed_dimen, 1, num_filters]
                W_filter = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1)) # initialize the filters' weights
                b_filter = tf.Variable(tf.constant(0.1, shape=[num_filters]))  # initialize the filters' biases
                ...
                # The result of our embedding doesnâ€™t contain the channel dimension
                # So we add it manually, leaving us with a layer of shape [None, sequence_length, embedding_size, 1].
                x_embed_expanded_word = tf.expand_dims(x_embed_word, -1)
                conv = tf.nn.conv2d(x_embed_expanded_word, W_filter, strides=[1, 1, 1, 1], padding="VALID")
                # Apply nonlinearity
                h = tf.nn.relu(tf.nn.bias_add(conv, b_filter), name="relu")
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(1, n_epochs + 1):
                # model training
                n_batch = 0
                for X_batch_charidx, X_batch_mask, X_batch_wordembed, domain_actual_lens, X_batch_suf, sample_weights, y_batch in self.next_batch(self.domains_train):
